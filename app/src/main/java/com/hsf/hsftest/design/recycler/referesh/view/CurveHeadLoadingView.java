...
import com.hsf.hsftest.util.utils.DensityUtil;
...
public class CurveHeadLoadingView extends View {
    ...
    private static final int DEFAULT_TEXT_PAINT_SIZE_SP = 13;
    private int mPaintSize;// = 40;
    private int mPaintTextBaseLine; //BASELINE的高度
    ...
    void initPaint() {
        mPaintSize = DensityUtil.sp2px(DEFAULT_TEXT_PAINT_SIZE_SP);
        mPaintTextBaseLine = mPaintSize;
        mDefaultText = mContext.getString(R.string.app_name);
        mResText = mDefaultText;
        mTextPaint = new Paint();
        mTextPaint.setColor(Color.parseColor("#666666"));
        mTextPaint.setTextSize(mPaintSize);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(mTextWidth, mPaintSize + baseLineSpace);
    }
     ...
     */
    void drawLinePath() {
        mPath.moveTo(0, mPaintTextBaseLine); //设定起始点
        mPath.lineTo(mTextWidth, mPaintTextBaseLine);//第一条直线的终点，也是第二条直线的起点
        mPath.close();
    }
    /**
     * 画出弧线路径
...
//        mPath.lineTo(mTextWidth/5,PAINT_TEXT_BASEIINE);
        mPath.quadTo(0, mPaintTextBaseLine, 5, mPaintTextBaseLine);
        mPath.quadTo(mTextWidth / 2, mPaintTextBaseLine + mRecfSpace, mTextWidth - 5, mPaintTextBaseLine);
        mPath.quadTo(mTextWidth * 5 / 6, mPaintTextBaseLine, mTextWidth, mPaintTextBaseLine);
        mPath.close();
    }
