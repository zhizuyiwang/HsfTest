...
public class Petal {
    ...
    private void draw(Point p, Canvas canvas) {
        if (!isFinished) {
            ...
            Point t = new Point(0, this.radius).rotate(PetalUtil.degrad(this.startAngle));
            //第一个端点，为了保证圆心不会随着radius增大而变大这里固定为3
            Point v1 = new Point(0, 3).rotate(PetalUtil.degrad(this.startAngle));
            //第二个端点
            Point v2 = t.clone().rotate(PetalUtil.degrad(this.angle));
            //延长线，分别确定两个控制点
